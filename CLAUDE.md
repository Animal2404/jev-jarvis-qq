...
挂在任意聊天 App 旁边（微信、QQ、X、飞书已适配）的非侵入式助手：读到对方最新消息 → 调 Jev 判断 → 悬浮窗给出分析和 3 条候选回复（Jev 排序）→ 人一键填入输入框。**发送永远由人手动点，程序不自动发。**
...
4. **不在本机构建**：本机没有 Android SDK，也不要装。构建一律推 GitHub，
   由 `.github/workflows/build-apk.yml` 云端跑。
5. **密钥不落盘、不进日志、不进 git、不进 APK**。任何文件里都不许出现真实密钥串；
   发布物是公开的，烧进去等于公开额度。
6. 编码一律 UTF-8。Python 读写文件必须显式 `encoding='utf-8'`。
7. `git commit` / `git push` 由人拍板后再执行。

## 技术栈

- Kotlin，传统 View + XML，**不用 Compose**
- minSdk 30，compileSdk / targetSdk 35
- 构建在 GitHub Actions：JDK 17（temurin）+ `android-actions/setup-android@v4`
  （platform 35 / build-tools 35.0.0），产物签名在无 release keystore 时回落 debug key
...
- Jev = TypeSafe 的判断模型，只回答选择题/打分/是非，不生成文字。**本项目走 Knox Studio**：
  `POST https://api.knoxstudio.ai/v1/systemone`，model `jev-1.13.0`，
  body `{model, state, questions}`，答案在 `answers`。响应结构与 OpenRouter 的
  `/alpha/decisions` 完全一致（`noul` / `choice`+`probabilities`+`confidence` / `score`+`legend`）。
  实测 7 题一次约 1.5 s。坑：**Jev 不能用 `/v1/chat/completions` 调**，网关会回
  `Jev models use POST /v1/systemone`。
- 回复生成走 TokenRhythm：`POST https://tokenrhythm.studio/v1/chat/completions`，
  model `mimo-v2.6-flash`（标准 OpenAI 兼容；实测接受 image_url，所以视觉/OCR 兜底也可用它）。
  **判断与回复是两家服务、两把密钥，不能互相回落。**
- Jev 主训练语言是英文：**题目的 instructions 和 criteria 用英文写，state 里的聊天内容保留中文原文。**
- **本项目不在本机构建**：没有 Android SDK，也不需要装。构建一律推 GitHub，由
  `.github/workflows/build-apk.yml` 在云端跑 `assembleRelease` 并发 Release。
- **API 密钥不进仓库、不进 APK**。发布物是公开的，烧进去等于公开额度；
  工作流里有断言：APK 内出现密钥特征串就直接构建失败。
