...
class Prefs(context: Context, prefsName: String = PREFS_MAIN) {
    ...
    /** "openrouter" | "typesafe" | "knox" | "custom". */
    var judgeProvider: String
        get() = sp.getString(K_JUDGE_PROVIDER, PROVIDER_KNOX) ?: PROVIDER_KNOX
    ...
    var judgeBaseUrl: String
        get() = sp.getString(K_JUDGE_BASE, DEFAULT_JUDGE_BASE_KNOX) ?: DEFAULT_JUDGE_BASE_KNOX
        set(v) = sp.edit().putString(K_JUDGE_BASE, v.trim()).apply()

    /**
     * The judge key, typed by the user in settings. It lives only in this
     * device's app-private storage; nothing is baked into the APK, so a
     * published build carries no usable credential.
     */
    var judgeKey: String
    ...
    var judgeModel: String
        get() = sp.getString(K_JUDGE_MODEL, DEFAULT_JUDGE_MODEL_KNOX) ?: DEFAULT_JUDGE_MODEL_KNOX
    ...
    var replyBaseUrl: String
        get() = sp.getString(K_REPLY_BASE, TOKENRHYTHM_BASE) ?: TOKENRHYTHM_BASE
        set(v) = sp.edit().putString(K_REPLY_BASE, v.trim()).apply()

    /**
     * Blank = fall back to [judgeKey]. Note the two default presets are *different*
     * services (Knox for judging, TokenRhythm for drafting), so on the defaults a
     * blank reply key sends the Knox key to TokenRhythm and 401s — the settings
     * page asks for both keys explicitly.
     */
    var replyKey: String
    ...
    var replyModel: String
        get() = sp.getString(K_REPLY_MODEL, TOKENRHYTHM_MODEL) ?: TOKENRHYTHM_MODEL
        set(v) = sp.edit().putString(K_REPLY_MODEL, v.trim()).apply()

    // --------------------------------------------------------------- vision

    /**
     * Defaults to the TokenRhythm preset: `mimo-v2.6-flash` accepts image_url
     * parts (verified), so the OCR fallback works on the same key as the reply
     * route. Still deliberately does NOT follow [replyBaseUrl] — a reply host
     * like DeepSeek has no vision endpoint, so inheriting it would break OCR.
     */
    var visionBaseUrl: String
        get() = sp.getString(K_VISION_BASE, TOKENRHYTHM_BASE) ?: TOKENRHYTHM_BASE
    ...
    /**
     * Reply route key: an explicit value wins, otherwise the judge key is
     * inherited. Note the two presets use *different* providers (Knox for
     * judging, TokenRhythm for drafting), so a user on the defaults must fill
     * the reply key too — the settings page says so.
     */
    fun effectiveReplyKey(): String = replyKey.ifBlank { judgeKey }

    /**
     * Vision route key, falling back to the reply route and then the judge
     * route.
     */
    ...
    fun judgeEndpoint(): String {
        ...
        return when (judgeProvider) {
            // TypeSafe and Knox Studio speak the same System One protocol; the
            // two presets differ only by host, key and model id. TypeSafe's box
            // holds the bare host (path appended), Knox's holds the `/v1` root
            // the user was given, so Knox only needs `/systemone` appended.
            PROVIDER_TYPESAFE -> "$base/v1/systemone"
            PROVIDER_KNOX -> "$base/systemone"
    ...
    /** Same shape as [replyEndpoint]; blank falls back to the TokenRhythm preset. */
    fun visionEndpoint(): String {
        val base = visionBaseUrl.trim().ifBlank { TOKENRHYTHM_BASE }
    ...
    companion object {
        ...
        const val PROVIDER_KNOX = "knox"
        const val PROVIDER_CUSTOM = "custom"

        const val OCR_MLKIT = "mlkit"
        const val OCR_VISION = "vision"

        // Judge route presets. Default is Knox Studio: the TypeSafe System One
        // protocol served from a host reachable in CN, model `jev-1.13.0`.
        ...
        const val DEFAULT_JUDGE_BASE_KNOX = "https://api.knoxstudio.ai/v1"
